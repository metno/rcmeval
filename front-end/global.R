## global.R - Load libraries and define help functions for the shiny app "rcmeval"
library(plotrix)
library(shiny)
library(shinyjs)
...
library(fields)
library(plotly)
library(rcmeval) # back-end package
...
data(package="rcmeval", "meta", envir=environment())
data(package="rcmeval", "statistics", envir=environment())
...
contactus <- function(name="", org="", from="", to="", body="") {
  if(nchar(body)>0 & nchar(to)>0) {
    subject <- paste0("New RCMeval comment from ",name," <",from,">")
