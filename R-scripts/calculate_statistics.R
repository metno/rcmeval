...
if(!require(rcmeval)) {
  ...
  library(devtools)
  install_github('metno/rcmeval/back-end')
}
library(rcmeval)
## To install rcmeval package from local repository: 
## R CMD INSTALL rcmeval/back-end

it.list <- list("present"=c(1985,2014), 
                "nf"=c(2041,2070), 
                "ff"=c(2071,2100))
...
if(calculate_statistics) {
  ...
  for(var in c("pr", "tas")) {
    for(it in it.list) {
      if(all(it==it.list$present)) {
        stats <- c("mean.gcm","spatial.sd.gcm","mean.ref","spatial.sd.ref","corr","rmse","cmpi")
        file.it <- paste("statistics.cmip",var,ref,paste(it,collapse="-"),"rda",sep=".")
        for(ref in switch(var, "pr"=ref.pr, "tas"=ref.tas)) {
          x <- calculate.statistics(files.gcm, meta=meta, file.out=file.it, path.out=path.out,
                                    ref=ref, path.ref=path.ref, period=it.list$present, 
        ...
        for(rcp in rcp.list) {
          file.it <- paste("statistics.cmip",var,ref,paste(it,collapse="-"),"rda",sep=".")
          files.rcp <- files.gcm[grepl(rcp,files.it)]
          x <- calculate.statistics(files.rcp, meta=meta, file.out=file.it, path.out=path.out,
                                    ref=NULL, path.ref=path.ref, period=it.list$present, 
                                    stats=stats, add=add, force=force, verbose=verbose)
        }
      }
    }
